...
static struct AdjListNode* newAdjListNode(int dest, int weight)
{
...
static void addEdge(struct Graph* graph, int src, int dest, int weight)
{
...
static int isEmpty(struct MinHeap* minHeap)
{
...
static void decreaseKey(struct MinHeap* minHeap, int v, int dist)
{
...
static bool isInMinHeap(struct MinHeap *minHeap, int v)
{
...
static void dijkstra(struct Graph* graph, int src)
{
...
void run_Dijkstra(uint8_t* seedIn, int seedSize) {
	Pair* arr=new Pair[V_COUNT*10];
	fillPairArray(seedIn, seedSize, arr, V_COUNT*10);
    Graph* graph = createGraph(V_COUNT);
	for(int i=0; i<V_COUNT*10; i++) {
		addEdge(graph, arr[i].a%V_COUNT, (arr[i].a>>16)%V_COUNT, int(arr[i].a>>32));
		addEdge(graph, arr[i].b%V_COUNT, (arr[i].b>>16)%V_COUNT, int(arr[i].b>>32));
	}
