...
void run_Kruskal(uint8_t* seedIn, int seedSize) {
    ...
    Graph* graph = createGraph(V_COUNT, V_COUNT*10);
	for(int i=0; i<V_COUNT*10; i++) {
	    graph->edge[i].src = arr[i].a%V_COUNT;
	    graph->edge[i].dest = (arr[i].a>>16)%V_COUNT;
	    graph->edge[i].weight = arr[i].b; 
	}
	KruskalMST(graph);
	for(int i=0; i<V_COUNT*10; i++) {
	    graph->edge[i].weight = ~graph->edge[i].weight;
	}
	KruskalMST(graph);
	delete[] arr;
}
